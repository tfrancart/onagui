...
public class MongeElkanAlignmentMethod<ONTORES1, ONTORES2> extends LabelAlignmentMethod<ONTORES1, ONTORES2> {
		
	public static final double DEFAULT_THRESHOLD = 0.90;
	
	private MongeElkan me;

	public MongeElkanAlignmentMethod(double current_threshlod) {
		me = new MongeElkan();
		me.setScaling(true);
		setThreshold(current_threshlod);
	}
	
	public MongeElkanAlignmentMethod() {
	...
	public Mapping<ONTORES1, ONTORES2> computeMapping(
		...
		for(LabelInformation oneLabelFrom1 : cpt1Labels) {
			for(LabelInformation oneLabelFrom2 : cpt2Labels) {
				...
				double score = me.score(rawLabel1, rawLabel2);
				if(score == 1.0) {
